...
void create(string *string_dest, char *chaine_src){
    ...
    string_dest->str = malloc(sizeof(chaine_src) + 1);
...
void add(string *chaine_dest, char c){
    char* nouveau = malloc(sizeof(chaine_dest->str) - 1 + sizeof(c));
    for (int i; i < chaine_dest-> N - 2; i++) {
        nouveau[i] = chaine_dest->str[i];
    }
    strcat(nouveau, c);
    strcat(nouveau, "\0");
    strcpy(chaine_dest->str, nouveau);
    chaine_dest->N = chaine_dest->N + strlen(c);
}
...
void delete(string *chaine_dest, int i){
    if (chaine_dest->N > i) {
        ...
        chaine_dest->str = realloc(chaine_dest->str, );
    }
